...
def find_repo_root() -> Path:
    """
    Find the HeatSentinal repository root robustly.

    Works whether this script is located at:
      HeatSentinal/download_ncr_weather.py
    or:
      HeatSentinal/ml/data_pipeline/download_ncr_weather.py

    You can also override the location with HEATSENTINEL_ROOT.
    """
    env_root = os.environ.get("HEATSENTINEL_ROOT")
    if env_root:
        root = Path(env_root).expanduser().resolve()
        if (root / "backend" / "data" / "ncr_boundary.geojson").exists():
            return root
        raise FileNotFoundError(
            "HEATSENTINEL_ROOT was set, but the NCR boundary was not found at: "
            f"{root / 'backend' / 'data' / 'ncr_boundary.geojson'}"
        )

    script_path = Path(__file__).resolve()

    # Check the script directory and its parents. This supports both
    # repo-root placement and ml/data_pipeline placement.
    for candidate in [script_path.parent, *script_path.parents]:
        if (candidate / "backend" / "data" / "ncr_boundary.geojson").exists():
            return candidate

    raise FileNotFoundError(
        "Could not locate the HeatSentinal repository root. Expected to find "
        "backend/data/ncr_boundary.geojson in the script directory or one of "
...
def main() -> int:
    ...
    print(f"NCR boundary:    {BOUNDARY_FILE}")
